	...
	function drawLayer(map:TiledMapData, tiles:h2d.Tile, layer:Int, size:Int) {
		...
		if (map.layers.length > 0 && layer < map.layers.length) {
			...
			for (tileId in map.layers[layer].data) {
				// Tile stores empty tiles as 0 and offsets the tileId by 1 so we must skip empty tiles and adjust the tileId back to the proper index
				if (tileId > 0 && tileId < tileSetArray.length)
					tileGroup.add(tileX, tileY, tileSetArray[tileId - 1]);
				tileX += size;
